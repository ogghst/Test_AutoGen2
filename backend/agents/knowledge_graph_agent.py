...
from autogen_core.models import SystemMessage, Message
    ...
    async def handle_task(self, message: UserTask, ctx: MessageContext):
        ...
        if self._plan is None:
            ...
            try:
                ...
                await self.publish_message(AgentResponse(context=[Message(content=error_msg)], reply_to_topic_type=self._agent_topic_type), topic_id=ctx.reply_to)
                return
        ...
        if transfer_tool:
            ...
            await self.publish_message(AgentResponse(context=[Message(content=final_message)], reply_to_topic_type=self._agent_topic_type), topic_id=ctx.reply_to)
